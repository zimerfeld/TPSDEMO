...
# Sessão online ativa = peer de rede que NÃO é o OfflineMultiplayerPeer (host ENet ou cliente).
...
func _apply_emergency(reason: String) -> void:
	Engine.physics_ticks_per_second = physics_throttle_tps
	# Em sessão ONLINE nunca pausamos a árvore: get_tree().paused congela o MultiplayerSpawner/
	# Synchronizer e os RPCs → o player do cliente não nasce (sem câmera = tela preta) e nada
	# sincroniza em NENHUMA sala. Estrangula a física e avisa, sem pausar. O pause (com overlay)
	# fica só p/ single-player/offline, onde não há netcode para quebrar.
	if _is_online_session():
		emergency_activated.emit(reason)
		push_error("[StabilityGuard] EMERGENCY (online — física estrangulada, sem pausar): " + reason)
		return
