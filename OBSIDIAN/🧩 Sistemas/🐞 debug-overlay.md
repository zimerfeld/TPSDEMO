...
> próprios sobre o preview — ver [[🗿 biblioteca-de-modelos]]. O overlay GLOBAL aplica **só o
  ...
  **Tab = branco** — `_LINE_COLORS`). **Debug 2D ligado sozinho não mostra nada**: borda/tooltips
  ...
  - **Linha Tab** (`ShowTabRow` → `show_tab`, **última** sub-linha, abaixo de `ShowPathRow`): mostra o
    **índice de Tab/foco** de cada controle (`TAB: n`, ou `TAB: -` se não focável). **Valor ESPERADO em
    ...
    [[🔁 navegacao-tab]]), a linha exibe esse número (`UINav.tab_order_of`), para a ordem ser
    ...
    - **Por que alguns controles ficam `TAB: -`?** Dois motivos: (1) **não são focáveis** — `Label`,
      ...
      (`UINav.wire_tab_ring(self)`) faz todos receberem `TAB: 1..N`. Detalhe em [[🔁 navegacao-tab]].
...
- **Janela flutuante aberta → some o overlay da UI de fundo (2026-06-27):** enquanto QUALQUER
  ...
  visibilidade ao vivo (ver [[🩸 dano-localizado]], [[🗿 biblioteca-de-modelos]]).
...
então o overlay global não a toca em 3D). Ver [[🗿 biblioteca-de-modelos]].

Relacionado: [[🩸 dano-localizado]] (mesmo classificador `BodyParts`),
[[🗣️ localizacao]], [[📌 ancoragem-ui]], [[🗿 biblioteca-de-modelos]].
