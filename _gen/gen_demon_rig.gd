...
const OUT_DIR := "res://library3D/characters/demonio_rig"
const OUT_PATH := OUT_DIR + "/demonio_rig.tscn"
const SIDES := 10   # lados de cada anel dos tubos
...
func _build() -> Node3D:
	...
	# ---- Acessórios rígidos presos a ossos (como a arma do player) ----
	_accessories(skel)
...
func _build_body_mesh() -> void:
	# Tronco: hips -> spine -> chest -> base do pescoço.
	_chain([
		_cp("hips", 0.17, [["hips", 1.0]]),
		_cp_at(Vector3(0.0, 0.97, 0.0), 0.18, [["hips", 0.5], ["spine", 0.5]]),
		_cp("chest", 0.19, [["spine", 0.4], ["chest", 0.6]]),
		_cp_at(Vector3(0.0, 1.26, 0.0), 0.14, [["chest", 1.0]]),
	])
	# Pescoço -> cabeça.
	_chain([
	...
	for s in ["L", "R"]:
		_chain([
			_cp("shoulder." + s, 0.10, [["shoulder." + s, 1.0]]),
			_cp_mid("shoulder." + s, "forearm." + s, 0.45, 0.085, [["shoulder." + s, 1.0]]),
			_cp("forearm." + s, 0.075, [["shoulder." + s, 0.5], ["forearm." + s, 0.5]]),
			_cp_mid("forearm." + s, "hand." + s, 0.5, 0.065, [["forearm." + s, 1.0]]),
			_cp("hand." + s, 0.06, [["forearm." + s, 0.4], ["hand." + s, 0.6]]),
		])
		_sphere(_head[_bi["hand." + s]] + Vector3(0.0, -0.04, 0.0), 0.075, [["hand." + s, 1.0]])

	# Pernas (quadril -> joelho -> tornozelo) + pés.
	for s in ["L", "R"]:
		_chain([
			_cp_at(_head[_bi["thigh." + s]] + Vector3(0.0, 0.04, 0.0), 0.12, [["hips", 0.5], ["thigh." + s, 0.5]]),
			_cp_mid("thigh." + s, "shin." + s, 0.5, 0.105, [["thigh." + s, 1.0]]),
			_cp("shin." + s, 0.09, [["thigh." + s, 0.5], ["shin." + s, 0.5]]),
			_cp_mid("shin." + s, "foot." + s, 0.55, 0.08, [["shin." + s, 1.0]]),
			_cp("foot." + s, 0.07, [["shin." + s, 0.4], ["foot." + s, 0.6]]),
		])
		_foot(s)
