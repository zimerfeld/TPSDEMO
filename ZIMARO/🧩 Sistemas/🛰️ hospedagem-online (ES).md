...
- ENet corre sobre **UDP**. El puerto por defecto es **UDP 44000** (`playonline.tscn`, `Port` SpinBox, `value = 44000`) — el mismo puerto del túnel playit del proyecto. Era `4383` hasta el 2026-08-06.
- El campo **Address** acepta un hostname, así que puedes usar dominios en lugar de una IP. Una **pista de formato** bajo el campo (`AddressFormatHint`) lo indica: "Acepta dominio (ej.: `zimaro.playit.game`) o IP — el puerto va en el campo Puerto."
- **Resolución de DNS asíncrona (2026-08-06).** El `create_client` de ENet resuelve hostnames por su cuenta, pero de forma **bloqueante** — en una red lenta el frame se congela hasta que responde el DNS y el juego parece colgado. Ahora `_on_join_rooms_pressed` solo llama a `create_client` con una **IP ya resuelta**:
  - IP literal (`is_valid_ip_address()`) → conecta directo, no hay nada que resolver.
  - Hostname → `IP.resolve_hostname_queue_item(host, IP.TYPE_ANY)` (cola del resolver, corre en un hilo) y `_process` consulta el estado con `_poll_resolve()` sin bloquear; al terminar llama a `_start_client(ip, host)`.
  - **Preferencia por IPv4** (`_pick_address`): playit publica A **y** AAAA y el resolver suele devolver primero la IPv6 (`zimaro.playit.game` → `2602:fbaf:824:1::1d` y `147.185.221.29`). Conectar por IPv6 dejaría fuera a quien no tiene ruta IPv6, así que elegimos la primera dirección **sin** dos puntos; la IPv6 solo se usa si no hay IPv4.
  - **Etiqueta de estado** (`ConnectStatus`, grupo `loc_manual`): "Resolviendo dirección…" → "Conectando…", limpiada al abortar. Un fallo de DNS muestra su propio aviso (`MSG_RESOLVE_FAILED`) con el nombre tecleado; un campo vacío avisa antes de intentarlo (`MSG_EMPTY_ADDRESS`).
  - **El estado y "Cargando" nunca se solapan (2026-08-06).** El panel de *Cargando* (con su barra de progreso) ya no aparece junto a "Conectando…": `_start_client` solo prepara el peer y las señales; `loading.show()` (y `_clear_status()`) pasaron a `_on_client_connected_await_version`, que se ejecuta cuando ENet **cierra** la conexión. El orden visible queda *Resolviendo → Conectando →* (conectado) *→ Cargando*, este último cubriendo el handshake de versión hasta que se abre el navegador de salas.
  - El elemento de la cola se libera (`erase_resolve_item`) al terminar y en `_exit_tree`, para no filtrar nada entre entradas a la pantalla.
...
- **Historial de Puerto/IP:** junto a Port y Address hay un `OptionButton` ("Select…") con los **últimos 3 valores** usados. Persistido en `Settings.config_file` bajo la sección **`online`** (claves `ports` / `addresses`), recargado en `_ready` (`_refresh_history`). Seleccionar un elemento rellena el campo y el desplegable vuelve a "Select…".
  ...
  - Registrado: al pulsar **Host**/**Connect** (`_remember`), al pulsar **Enter** y cuando el campo **pierde el foco** (`_on_address_focus_exited` / `_on_port_focus_exited` — Port guarda vía el `get_line_edit().focus_exited` del SpinBox). Actualiza el desplegable de inmediato. Un dominio conecta directamente — resuelto antes en la cola asíncrona de `IP` (ver "Resolución de DNS asíncrona" arriba).
