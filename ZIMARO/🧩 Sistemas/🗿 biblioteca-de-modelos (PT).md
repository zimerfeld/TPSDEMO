...
Ver [[🔌 MCP do Godot (PT)|🔌 MCP do Godot]].

## Fluxo da tela

**Categoria → Modelo → Parte**, com **seleção sequencial** (2026-06-16; o dropdown
**Prefixo** foi removido em 2026-06-30 — ao escolher a Categoria, o combo Modelo já
lista **todos** os modelos da categoria, sem filtro intermediário):
...
default — ver [[🔽 dropdowns (PT)|🔽 dropdowns]]); selecioná-lo recarrega/destrava os
...
>   (o dropdown **Prefix/Prefixo** foi **removido** em 2026-06-30),
...
> [!note] Rótulos gateados pelo dropdown em "Selecione..." (2026-07-04)
> Cada família de rótulo 3D só aparece quando o SEU dropdown tem um item escolhido (≠ "Selecione..."),
> de modo uniforme:
> - **Sub-membro** (`_refresh_sub_member_labels`) e **Esqueleto** (`_refresh_aux_labels`) já faziam isso
>   (retorno cedo com `cbo_sub_members.selected <= 0` / `cbo_skeleton.selected <= 0`).
> - **Membro** era a exceção: com o dropdown Membro em "Selecione..." o stack "Membro:" aparecia sobre
>   TODOS os membros. Agora `_apply_member_labels_visibility` faz o mesmo gate — `if member_row.visible
>   and cbo_members.selected <= 0: return` (limpa as pilhas e não recria). "Todos os membros" (índice 1) e
>   um membro específico (índices 2+) continuam exibindo; só o placeholder oculta. Como as linhas
>   Tipo/Nome/ID moram no MESMO stack ancorado ao corpo do membro, elas também somem sem membro escolhido.
...
> **topo direito, ao lado do título** (que tem **tooltip de Debug 2D**). Ver [[🐞 debug-overlay (PT)|🐞 debug-overlay]].
...
- **Colisores** — `_apply_colliders_visibility()` adiciona/remove os gizmos wireframe
  ...
  - **Geometria do collider + janela de Afastamento/Escala por membro/sub-membro/osso avulso (2026-06-25,
    ...
    **Sub-membro** ou **Esqueleto**, aparece **à direita** daquele dropdown um **dropdown de geometria**
    ...
    ([[📌 ancoragem-ui (PT)|FloatingWindow]] dos controles2D) — `_open_or_update_collider_dialog`,
...
- **Dano** — aberto pelo **botão `DamageButton`** (à direita do "Voltar", em `UI/Actions`; texto "Dano"/"Damage").
  ...
  - **ÁRVORE (Tree) NA janela (2026-06-21):** `_refresh_damage_panel` constrói um `Tree`
    ...
    o ícone de lixeira não encostar na linha vizinha. Ver [[🩸 dano-localizado (PT)|🩸 dano-localizado]].
  ...
  - **Editor de IA virou `FloatingWindow` (2026-06-30):** o painel IA deixou de ser um `PanelContainer`
    ...
    foco/ESC, supressão do Debug 2D de fundo e o click-forward do toggle (ver [[🐞 debug-overlay (PT)|🐞 debug-overlay]]).
  ...
  - **Subseção "Sub-membros" (2026-06-20):** cada sub-membro existente é uma linha (rótulo + `SpinBox`
    ...
    como antes; só os `PART_*` ganham esta subseção. Ver [[🩸 dano-localizado (PT)|🩸 dano-localizado]]. A const
  ...
  - **Sob qual MEMBRO o sub-membro aparece (2026-06-21):** o dropdown "Sub-membro" agrupa cada
    ...
    lista "Adicionar sub-membro". Ver [[🩸 dano-localizado (PT)|🩸 dano-localizado]].
...
(Os emissores vão pro bus `SFX` — ver [[🔊 audio (PT)|🔊 audio]].) Os estados de **rotação,
...
Além dos toggles, **toda escolha dos dropdowns** (Categoria · Modelo · Parte ·
...
categoria, `sel_model` = nome do modelo, `sel_part` = rótulo da
...
- **valor vazio** (o usuário parou ali) → deixa o combo no placeholder **habilitado**, pronto p/
  ...
  auto-selecionado — ver [[🔽 dropdowns (PT)|🔽 dropdowns]]).
...
membro (via [[🦿 limb-colliders-gd (PT)|LimbColliders]]) e `_add_member_labels`
...
> dos demais membros, para **cobrir toda a malha** da cabeça. Ver [[🩸 dano-localizado (PT)|🩸 dano-localizado]].
...
> pular (ver o item de desempenho em "Colisores"). Ver [[🩸 dano-localizado (PT)|🩸 dano-localizado]].
