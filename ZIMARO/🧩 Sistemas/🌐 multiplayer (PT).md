...
`level_1` e `level_2` têm `MultiplayerSpawner` (spawn_path → `SpawnedNodes`)
...
- **Modo "Hospedar Somente"** (`PlayerSelection.spectator_host`): com `spawn_host=false`, o **`NetSpawn`**
  adiciona uma **câmera livre não replicada** em vez do player do host.
  Centralizado em `NetSpawn._add_spectator_camera` **(2026-06-24)** → funciona nos níveis (antes
  isso era tratado por um só nível; os demais ignoravam e criavam player mesmo no "Hospedar Somente").
  Ver [[🛰️ hospedagem-online (PT)|🛰️ hospedagem-online]].
...
- Os níveis são escolhidos no fluxo **Jogar Online** (chooseplayer → levels → playonline);
  ver [[🎬 fluxo-de-cenas (PT)|🎬 fluxo-de-cenas]].
...
- Cada nível chama **`NetSpawn.setup(spawned_nodes, player_spawn_points, not PlayerSelection.spectator_host)`**
  no `_ready` (substitui o `add_player`/`del_player` duplicado nos níveis). Os níveis passam
  ...
  - **Cenário em execução**: `NetSpawn` guarda o caminho da cena do nível (`_server_scenario`) e o
    ...
    para só confirmar reconexão quando o cliente entra no **mesmo cenário**. Ver [[🛰️ hospedagem-online (PT)|🛰️ hospedagem-online]].
...
- [[🛰️ hospedagem-online (PT)|🛰️ hospedagem-online]] — jogar com amigos pela internet (playit.gg / Tailscale / port forwarding)
