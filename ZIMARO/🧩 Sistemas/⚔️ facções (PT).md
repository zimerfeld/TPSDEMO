...
distinto do [[🧠 red-robot-ai-gd (PT)|AIConfig]], que guarda uma facção por-**modelo**, estática, em JSON.
...
- **Sem fogo amigo (dano)** — `bullet.gd` e `bomb.gd`:
  ...
  - A **bala atravessa** o aliado (exceção de colisão) em vez de explodir → não bloqueia a linha de
    tiro de quem está atrás. Ver [[🔫 combate-tiro (PT)|🔫 combate-tiro]].
...
- **Targeting (alvo)** — inimigos miram só a facção OPOSTA, filtrando por `are_enemies` na **seleção**
  ...
  - `red_robot._pick_target` (ver [[🤖 inimigos (PT)|🤖 inimigos]]), `criatura_alada._find_nearest_player`.
  - `player_bot_ai._is_valid_enemy` / `_is_ally` (bot aliado só engaja hostis; ver [[🎮 player (PT)|🎮 player]]).
...
- **Separação entre aliados (2026-07-08):** vários bots aliados não se empilham mais — cada um usa
  **separation steering** (empurrão para longe dos outros aliados dentro de `separation_radius`),
  distribuindo-os na órbita/combate. Detalhe em [[🎮 player (PT)|🎮 player]].
...
- [[🤖 inimigos (PT)|🤖 inimigos]] · [[🎮 player (PT)|🎮 player]] · [[🔫 combate-tiro (PT)|🔫 combate-tiro]] · [[🧠 red-robot-ai-gd (PT)|🧠 red-robot-ai-gd]]
