...
apenas orquestram os nós nativos. Ver [[🩸 dano-localizado (PT)|🩸 dano-localizado]] e
[[🦿 limb-colliders-gd (PT)|🦿 limb-colliders-gd]] para o exemplo canônico: um `Node3D` que
...
| Nó/Recurso | Uso |
|---|---|
| `AudioStreamPlayer3D` (13×) · `AudioStreamPlayer` · `AudioStream`/`AudioStreamWAV` · `AudioStreamRandomizer` | emissores e streams. Ver [[🔊 audio (PT)|🔊 audio]] |

## Rede / multiplayer

| Nó/Recurso | Uso |
|---|---|
| `MultiplayerSynchronizer` (7×) · `MultiplayerSpawner` · `SceneReplicationConfig` | `player_input.gd` herda `MultiplayerSynchronizer`. Ver [[🌐 multiplayer (PT)|🌐 multiplayer]] |
...
| Subclasses de `Control` | widgets de HUD em `controls2D/` |
