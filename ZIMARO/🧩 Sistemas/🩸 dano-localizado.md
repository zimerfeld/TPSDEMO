...
(ver [[🦴 body-parts-gd]]). ⚠️ **Detalhe técnico:** o PT **"pe"** (pé) é casado por **TOKEN EXATO**
...
[[🦴 body-parts-gd]] para a nota dedicada.
...
- **`model_key`** = nome da pasta do modelo (`"red_robot"`, `"player"`), o MESMO valor que
  ...
  Todos por membro/sub-membro, editáveis na tela Models (ver [[🗿 biblioteca-de-modelos]]);
...
- **Cache de leitura `_entry_cache` (2026-07-23) — desempenho:** antes, CADA getter
  ...
  projeto em [[🗿 biblioteca-de-modelos]].
- **Editor:** a tela Models tem o toggle **"Dano"** (renomeado de "Dano por membro" em 2026-06-22) que abre um painel flutuante
  ...
  em "Modelo completo"**. Ver [[🗿 biblioteca-de-modelos]]. `res://` é gravável só rodando
...
grande "Remover sub-membro" do rodapé — ver [[🗿 biblioteca-de-modelos]]). No fim, a linha
...
`_refresh_damage_panel` quanto em `_on_language_changed` (ver [[🗣️ localizacao]]).
...
> Verificado via MCP do Godot ([[🔌 MCP do Godot]]): laser do enemy aplica 25 (arma),
