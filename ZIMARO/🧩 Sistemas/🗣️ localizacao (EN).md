...
# 🗣️ System — Localization (EN/PT/ES)

UI language switch between **Português**, **English** and **Español**, via the **Locale** autoload
(`autoload/locale.gd`). The supported languages live in `Locale.SUPPORTED_LANGS` (`["pt", "en", "es"]`).

## Per-scene dictionaries

Each scene has its **own trio** of flat JSONs in a `Resources/` folder next to the `.tscn`:
...
`scenes3D/models/Resources/models.pt.json` + `...en.json` + `...es.json`, etc. Each trio maps the
...
every `*.pt.json` / `*.en.json` / `*.es.json` and **merges** them all into a per-language table. To add
the `Resources/` dictionaries of a new screen you just drop them in — no need to edit the autoload.
(There is no longer a `pt.json`/`en.json`/`es.json` at the project root.)

## Persistence and application

- The choice is saved in `Settings` → `game/language` (`"pt"` default / `"en"` / `"es"`; a value outside
  `SUPPORTED_LANGS` falls back to the default).
...
The `UI/LangBar` (HBox in the bottom-right corner with "Português" / "English" / "Español" buttons, same
pattern as the menu) is present in **menu, chooseplayer, settings, developer, levels, playonline,
controls and models**. Each script calls `Locale.set_language(...)` and grays out the active language
button (`_update_language_buttons` — each screen has `portuguese_button`/`english_button`/`spanish_button`). Because the re-localization is in-place, the screen updates immediately.
...
`language_changed` signal. The HUD/Guard keys live in `scenes2D/overlays/Resources/overlays.{pt,en,es}.json`.
(The screens
...
`Resources/<scene>.pt.json`, `Resources/<scene>.en.json` AND `Resources/<scene>.es.json` of that scene,
in the same task.** Because the Locale indexes by the source text, changing the scene without updating the
key breaks the translation. PT gets the Portuguese translation; EN, the English one; ES, the Spanish one.
Validate all three JSONs at the end.
