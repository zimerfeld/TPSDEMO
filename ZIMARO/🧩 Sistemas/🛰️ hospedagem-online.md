...
- **Layout da tela** (`playonline.tscn`): cada input tem um **Label à esquerda** localizado — `Port:` (PT "Porta:") e `IP Address/Domain:` (PT "Endereço IP/Domínio:"), via `Resources/playonline.*.json`. **Abaixo** de Port/Address há dois botões (sem radios; clicáveis direto):
  - **ManageRooms** ("Gerenciar Salas", `_on_manage_rooms_pressed`) = papel **Host**: cria um servidor ENet **persistente** e abre o `host_session` (gerenciador de salas). Ver [[🚪 salas]].
...
a versão do host"*. O ID é carimbado no export pelo `build_windows.ps1` (`build_id.json` = SHA curto do git
+ `-dirty` + timestamp; lido por `RoomManager.game_version`, embutido no `.exe`). No **editor** todos batem
(`editor-dev`). Regra prática: **os dois têm que rodar o mesmo `.exe`** — agora versões diferentes avisam,
não travam mudo. Ver [[🌐 multiplayer]].
