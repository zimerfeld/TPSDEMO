...
| **Liberdade com coleira** (2026-08-06) | O posto rígido deixava o aliado **parado demais**. Agora ele anda para **qualquer direção** (orbitar, flanquear, recuar) dentro de uma **bolha** de `roam_radius` (4 m) em torno do protegido; passando dela, a volta entra proporcional ao excesso e vira dominante em `max_leash` (`_leash`). É a coleira — e não um posto fixo — que impede o afastamento, e ela também é o que impede a investida no inimigo. O posto de escolta virou **preferência** (`guard_screen_weight`), não âncora. Medido: **35,5 m percorridos**, máxima de **4,15 m** do protegido. |
| **Sem movimentos bruscos** (2026-08-06) | O rumo passa por `_smooth_dir` (interpolação exponencial, `move_dir_response`) — a IA pode mudar de ideia a cada varredura que o CORPO muda de direção gradualmente. Medido: **0 inversões bruscas** em 7 s. Mesmo remédio já existente no `red_robot` (`move_dir_response`), agora também na `criatura_alada`, cujo rumo horizontal era aplicado cru (o `look_at` virava de golpe a cada quadro). |
