...
- **Barra de botões no rodapé** (`UI/Actions` de menu/settings/models/chooseplayer/
  ...
  - **Também no menu** (sob `UI`) e nos **levels de gameplay** level_1/2/base (sob o `TitleCanvas`):
    ...
    — criadas para o toggle de Debug 2D injetado pelo `DebugOverlay`. Ver [[🐞 debug-overlay (PT)|🐞 debug-overlay]].
...
(settings.gd) e `Settings.apply_window_resolution` (config.gd). Ver [[🎬 fluxo-de-cenas (PT)|🎬 fluxo-de-cenas]].
...
tela 2D foi migrada para o esqueleto de containers; ver [[📐 layout-responsivo (PT)|📐 layout-responsivo]].

Relacionado: [[🔽 dropdowns (PT)|🔽 dropdowns]] · [[📐 layout-responsivo (PT)|📐 layout-responsivo]] · raiz de cena de UI deve
