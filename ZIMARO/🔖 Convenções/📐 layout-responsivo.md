...
sobreposição. Complementa [[📌 ancoragem-ui|ancoragem de UI]] (que cuida dos poucos
...
automaticamente antes de todo export** (ver [[🚀 Build Windows (Prod)]]).
...
- `Content` é um `HBoxContainer` com `Col2D`, `Col3D` e `PreviewPanel`, todos `size_flags_horizontal
  ...
  **mesma altura** das colunas automaticamente (sem `offset_top/bottom` fixos). Ver
  [[🐞 debug-overlay|preview do player]].
