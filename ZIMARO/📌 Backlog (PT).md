...
> Ligada ao [[🏠 Home (PT)|🏠 Home]]. Cada item aponta para a nota de sistema onde o contexto detalhado vive.
>
...
- **Extremidades viram sub-membros automáticos** — antebraço/mão (dono BRAÇO) e canela/pé (dono
  ...
  Ver [[🩸 dano-localizado (PT)|🩸 dano-localizado]] · [[🦴 body-parts-gd (PT)|🦴 body-parts-gd]] · [[🦿 limb-colliders-gd (PT)|🦿 limb-colliders-gd]].
...
- **Importação de modelos** — `humanoide` e `monstro_pedregulho3` corrigidos e reimportados (36
  ...
  importação. Nota nova [[🔌 MCP do Godot (PT)|🔌 MCP do Godot]] documenta o addon e os comandos headless.
...
- **Locomoção terrestre sem deslizar:** passada da `Walk` medida em runtime (~0,8 m/s), cadência via
  ...
  motion), velocidades da IA reduzidas. Ver [[🤖 inimigos (PT)|🤖 inimigos]].
...
- **Sistema de facções (runtime, por-instância):** novo `effects_shared/factions.gd`. Sem fogo amigo
  (bala atravessa aliado), targeting por facção, neutros dinâmicos (~8 s). Ver [[⚔️ facções (PT)|⚔️ facções]].
- **Aliado orbita o player mais próximo sem colidir** (`player_bot_ai`): âncora no humano mais perto,
  órbita (raio + tangencial), exceção de colisão bot↔âncora. Ver [[🎮 player (PT)|🎮 player]].
- ✅ **Separação entre múltiplos bots aliados (2026-07-08):** `player_bot_ai._separation` (steering
  ...
  sem empilhar. Ver [[🎮 player (PT)|🎮 player]] / [[⚔️ facções (PT)|⚔️ facções]].
...
**Procedimento documentado no cofre:** [[📦 Atualizar Asset da Release (GitHub) (PT)|📦 Atualizar Asset da Release (GitHub)]] (PT/EN)
...
[[🧩 templates-de-level (PT)|🧩 templates-de-level]]):
...
combate**. Detalhes: [[🌌 ambiente-dos-levels (PT)|🌌 ambiente-dos-levels]]. READMEs (3) atualizados. O exe caiu de
...
([[🏠 Home (PT)|🏠 Home]], [[🎬 fluxo-de-cenas (PT)|🎬 fluxo-de-cenas]] ordem de Tab da `levels`, [[🚪 salas (PT)|🚪 salas]] nota histórica,
[[📄 formatacao (PT)|📄 formatacao]]). **Falta só:** rodar `build_windows.ps1` (feito ao fim da sessão) e a
...
- **Áudio:** `audios/level_base.ogg` removido (+ `.import`); conferir se `MusicManager`/atribuições por
  cena não referenciam mais esse arquivo. Ver [[🔊 audio (PT)|🔊 audio]].
...
usuário commitar/publicar** (não commitar). Sistemas: [[🚪 salas (PT)|🚪 salas]], [[🗿 biblioteca-de-modelos (PT)|🗿 biblioteca-de-modelos]].
...
- **Navegação em cascata** no Gerenciador de Templates (characters) e o novo **Gerenciador de
  Cenários** (sceneries) — ver [[🧩 templates-de-level (PT)|🧩 templates-de-level]]. Tela `levels` com 2 botões por
...
- **Tela `levels` em grade responsiva**: `GridContainer` 3 colunas (Level fixo 300 px | Template |
  ...
  meio**, espaçamento uniforme 16 px nas duas linhas. Ver [[📐 layout-responsivo (PT)|📐 layout-responsivo]].
- **`build_windows.ps1` blindado**: apaga `.godot/exported/` antes de todo export — o cache não
  ...
  com sondas headless e marcadores no binário). Ver [[🚀 Build Windows (Prod) (PT)|🚀 Build Windows (Prod)]].
...
1,800; base 0,000 — 3/3 OK). Detalhes: [[🩸 dano-localizado (PT)|🩸 dano-localizado]] · [[🎮 player (PT)|🎮 player]]. Respondeu à
...
- **Pulo variável (hold/release):** segurar **espaço** = animação do salto completa + distância máxima
  ...
  pulam (IA), logo não são afetados. Ver [[🎮 player (PT)|🎮 player]].
...
- ✅ **Protocolo de teste autossuficiente** criado: [[🧪 teste-salas-multiplayer (PT)|🧪 teste-salas-multiplayer]] (loopback local
...
- ✅ **Teste A (loopback `127.0.0.1`, 2 instâncias) VALIDADO em campo (2026-07-02):** host cria sala,
  ...
  (não nascer numa sala parada durante o `chooseplayer`). Ver [[🧪 teste-salas-multiplayer (PT)|🧪 teste-salas-multiplayer]] · [[🚪 salas (PT)|🚪 salas]].
...
Contexto completo: [[🚪 salas (PT)|🚪 salas]] · [[🌐 multiplayer (PT)|🌐 multiplayer]] · [[🛰️ hospedagem-online (PT)|🛰️ hospedagem-online]].
...
andamento) e **parametrização** (UI, adiada por decisão do usuário). Contexto: [[🤖 inimigos (PT)|🤖 inimigos]],
[[🧠 red-robot-ai-gd (PT)|🧠 red-robot-ai-gd]], `red_robot_ai.gd` / `criatura_alada_ai.gd` / `red_robot.gd`.

**Comportamento (código) — ✅ FEITO (2026-07-02, ver [[🤖 inimigos (PT)|🤖 inimigos]] "Refinamento de IA"):**
...
- Base de UI já existe: `FloatingWindow`/`FloatingDialog` (mesmo padrão modal/estilo). Os parâmetros hoje são
  ...
  numéricos por modelo. Ver [[🚪 salas (PT)|🚪 salas]] (janelas) e [[🧠 red-robot-ai-gd (PT)|🧠 red-robot-ai-gd]].
...
- **Layout responsivo (containers):** migrar as demais telas 2D do posicionamento por offsets absolutos
  ...
  Replicar para menu/settings/chooseplayer/levels/playonline/sessions. Ver [[📐 layout-responsivo (PT)|📐 layout-responsivo]].
...
- **`pause_menu`** (overlay `Control`, 3 botões + 3 sliders): ligar `UINav.wire_tab_ring(self)` +
  `grab_focus` inicial para não ficar fora do anel de Tab. É overlay de pausa (não troca de cena) → **opcional/secundário**. Ver [[🔁 navegacao-tab (PT)|🔁 navegacao-tab]] (§ Cobertura e pendências).

---

## Como retomar (checklist rápido)

1. Ler esta nota + [[🏠 Home (PT)|🏠 Home]] e as notas linkadas do item que vai atacar.
