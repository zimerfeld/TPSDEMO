...
- **Reimportar assets** (depois de trocar um `.glb`) — é o jeito CERTO, ver [[🗿 biblioteca-de-modelos]]:
...
- **Encerrar o jogo e o editor Godot** antes de mexer no código (regra do projeto; há hooks em
  `.claude/settings.json`). Ver [[💻 Rodar no Editor (Dev)]].
...
- **ESC não sai da tela (Models/outra):** a cena foi aberta **isolada** (`run_project` com `scene:`).
  ...
  ([[🧭 main-gd]]) — rodar o projeto **sem** especificar cena e navegar pelo menu.
...
## 🔗 Ligações
- [[💻 Rodar no Editor (Dev)]] — rodar/desenvolver pelo editor
